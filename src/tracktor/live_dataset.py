...
class IndividualDataset(torch.utils.data.Dataset):
    ...
    def append_samples(self, training_set_dict, frame=0, area=0):
        ...
        self.scores = torch.cat((self.scores, torch.zeros(len(training_set_dict['features']))))
        self.frame = torch.cat((self.frame, (torch.ones(len(training_set_dict['features'])*frame))))
    ...
    def __getitem__(self, idx):
        ...
        if self.flip_p > 0.0 and self.data_augmentation == 0:
            ## flip FM to augment
...
class InactiveDataset(torch.utils.data.Dataset):
    def __init__(self, data_augmentation=0, others_db=None, others_class=False, im_index=0,
                 ...
                 train_others=True, load_others=False, weightening=-1):
        self.boxes = torch.tensor([]).to(device)
        self.scores = torch.tensor([]).to(device)
        self.features = torch.tensor([]).to(device)
        self.weights = torch.tensor([]).to(device)
        self.max_occ = 0
        self.min_occ = 0
        ...
        self.occ = torch.tensor([]).to(device)
    ...
    def __getitem__(self, idx):
        ...
        return {'features': self.features[idx, :, :, :], 'scores': self.scores[idx], 'weights': self.weights[idx]}
    ...
    def get_others(self, inactive_tracks, val=False):
        ...
        train_others_weights = torch.tensor([]).to(device)
        ...
        if (num_tracks >= 2) or self.fill_up:
            ...
            if val:
                if num_tracks >= 4:  # enough sample to provide at least two IDs in val and train others
                    ...
                    # take all samples per ID in others
                    for i, idx in enumerate(train_idx_others):
                        for j in range(len(self.others_db[idx])):
                            train_others_features = torch.cat((train_others_features, self.others_db[idx][j][1].unsqueeze(0).to(device)))
                    if self.train_others:
                        self.occ = torch.cat((self.occ, torch.ones(1).to(device) * train_others_features.shape[0]))
                    else:
                        self.occ = torch.cat((self.occ, torch.ones(1).to(device) * -1))
                    for i, idx in enumerate(val_idx_others):
                        for j in range(len(self.others_db[idx])):
                            val_others_features = torch.cat((val_others_features, self.others_db[idx][j][1].unsqueeze(0).to(device)))
                    return train_others_features, val_others_features, train_others_frames_id, val_others_frames_id, fill_others_features, fill_others_id

                    # if number according to samples of tracks
                    c = 0
            ...
            #### no validation set
                ...
                if self.others_class:
                    ...
                    for i, idx in enumerate(train_idx_others):
                        ...
                        if not self.upsampling and not self.weightedLoss and True:
                           ...
                           for j in range(len(self.others_db[idx])):
                               train_others_features = torch.cat(
                                   (train_others_features, self.others_db[idx][j][1].unsqueeze(0).to(device)))
                               # weight is 1/number of others * 1/samples per others
                               if self.weightening == 0:
                                   pass
                               elif self.weightening == 1:
                                   weight = torch.ones(len(self.others_db[idx])).to(device) /\
                                                           ((len(inactive_tracks) + 1) * len(self.others_db[idx]) * len(train_idx_others))
                                   train_others_weights = torch.cat((train_others_weights, weight))
                               else:
                                   print(f"weight not implemented")
                                   exit()
                    ...
                    if self.weightening == 0:
                        if self.train_others:
                            train_others_weights = torch.ones(len(train_others_features)).to(device) / len(train_others_features)
                    if not self.train_others:
                            train_others_weights = torch.zeros(len(train_others_features)).to(device)
                ...
                return train_others_features, val_others_features, train_others_frames_id, val_others_frames_id, fill_others_features, fill_others_id, train_others_weights

        else:
            print('\n no others dataset. num tracks: {}'.format(num_tracks))
            return torch.tensor([]).to(device), torch.tensor([]).to(device), torch.tensor([]).to(device), torch.tensor([]).to(device), torch.tensor([]).to(device), torch.tensor([]).to(device), torch.tensor([]).to(device)
    ...
    def get_val_set(self, val_idx, inactive_tracks, val_others_features, val_others_fId):
        ...
        if val_others_features.shape[0] > 0:
            ...
            val_set.occ = torch.cat((val_set.occ, torch.ones(1).to(device) * val_others_features.shape[0]))
        # else:
        ...
        for i, idxs in enumerate(val_idx):
            ...
            val_set.occ = torch.cat((val_set.occ, torch.ones(1).to(device) * r['scores'].shape[0]))
    ...
    def get_training_set(self, inactive_tracks, tracks, val, split, val_set_random, keep_frames):
        ...
        if get_others:
            ...
            train_others_features, val_others_features, train_other_fId, val_others_fId, fill_features, fill_id, train_others_weights = self.get_others(inactive_tracks, val)
            ...
            self.frame = torch.cat((self.frame, train_other_fId))
            self.weights = torch.cat((self.weights, train_others_weights))
            ...
            self.occ = torch.cat((self.occ, torch.ones(1).to(device)))
        ...
        for i, t in enumerate(inactive_tracks):
            ...
            pos_unique_indices = self.expand_indices_augmentation(t.training_set.pos_unique_indices) if\
                self.data_augmentation > 0 else  t.training_set.pos_unique_indices# if data augmentation is used

            r = t.training_set[pos_unique_indices]
            if self.weightedLoss:
                self.occ = torch.cat((self.occ, torch.ones(1).to(device) * r['scores'].shape[0]))
            ...
            self.frame = torch.cat((self.frame, torch.cat((r['frame_id'].unsqueeze(1),
                                                          ...
                                                          dim=1)))
            if self.weightening == 0:
                self.weights = torch.cat((self.weights, torch.ones(r['scores'].shape[0]).to(device) / r['scores'].shape[0]))
            elif self.weightening == 1:
                self.weights = torch.cat((self.weights, torch.ones(r['scores'].shape[0]).to(device) / ((len(inactive_tracks) + 1) * r['scores'].shape[0])))
